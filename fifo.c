...
static FILE *openOutPipe(char *name) {
    ...
    if (pipe == NULL) {
        logger_log(LOGGER_LOG_LEVEL_ERROR, "Error in creating output pipe");
        exit(-1);
    }
    return pipe;
}
...
static FILE *openInPipe(char *name) {
    ...
    pipe = fopen(name,"r+");
    if (pipe == NULL) {
        logger_log(LOGGER_LOG_LEVEL_ERROR, "Error in creating input pipe");
        exit(-1);
    }
    int flags, fd;
    fd = fileno(pipe);
...
void sendStringToPipe(PipesPtr pipes, const char *data) {
    ...
    if (result == 0)
        logger_log(LOGGER_LOG_LEVEL_ERROR, "Failed to send data");
}
